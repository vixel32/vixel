...
export default function AdminProducts() {
  ...
  const handleUpload = async (files: FileList) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const fileName =
        `product-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (productId && newImageUrls.length > 0) {
      ...
      if (imageErr) {
        setError(`Produk tersimpan, tetapi gambar gagal disimpan: ${imageErr.message}`);
        setSaving(false);
        return;
      }
    }
  ...
  const handleImportFile = async (file: File) => {
    ...
    try {
      const data = await importProductsFromExcel(
        file,
        categories
      );
  ...
  const confirmImport = async () => {
    ...
    for (const item of importedData) {
      ...
      if (!cat) {
        errors.push(
          `Kategori "${item.category_name}" tidak ditemukan untuk produk "${item.name}".`
        );
        continue;
      }
      ...
      if (existing) {
        ...
        if (updErr) {
          errors.push(
            `Gagal update "${item.name}": ${updErr.message}`
          );
        } else {
          if (item.image_url) {
        ...
        if (insErr) {
          errors.push(
            `Gagal tambah "${item.name}": ${insErr.message}`
          );
        } else {
          if (item.image_url) {
            await supabase
  ...
  useEffect(() => {
    if (
      themeFilter &&
      !availableThemes.includes(themeFilter)
    ) {
      setThemeFilter('');
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() =>
              downloadImportTemplate(categories)
            }
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          title={
            categoryFilter
              ? 'Tema yang tersedia mengikuti kategori yang dipilih'
              : 'Semua tema tersedia'
          }
        >
      ...
      {loading ? (
        ...
        <div className="card py-16 text-center">
          ...
          <p className="text-sm text-charcoal-400 mt-1">
            Coba ubah kata pencarian, tema, atau kategori.
          </p>
              ...
              return (
                ...
                >
                  <div className="relative aspect-[4/3] bg-cream-100">
                    ...
                    {product.is_featured && (
                      <span className="absolute top-2 left-2 inline-flex items-center gap-1 rounded-full bg-gold-400 px-2 py-0.5 text-xs font-semibold text-charcoal-900">
                        <Star className="w-3 h-3 fill-charcoal-900" />
                        Unggulan
                      </span>
                    )}
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <h3 className="font-serif text-base font-semibold text-charcoal-800 mt-0.5 line-clamp-1">
                      {product.name}
                    </h3>
                    ...
                    <p className="text-sm font-semibold text-charcoal-700 mt-1">
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        ...
                        className="btn text-xs px-3 py-2 bg-navy-50 text-navy-700 hover:bg-navy-100"
                      >
      ...
      {showForm && (
        <div
          className="fixed inset-0 z-[60] bg-charcoal-900/60 flex items-center justify-center p-4"
          onClick={() =>
            setShowForm(false)
          }
        >
            ...
            >
              ...
              <div>
                  ...
                  {allImages.map((url) => (
                    ...
                    >
                      <img
                        ...
                        alt=""
                        referrerPolicy="no-referrer"
                        className="w-20 h-20 rounded-xl object-cover border border-charcoal-100"
                      />
                ...
                <p className="text-xs text-charcoal-400 mt-1">
                  Upload file langsung, pilih dari Google Drive,
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div className="mt-3 flex items-center gap-3 rounded-xl bg-navy-50 border border-navy-100 p-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-charcoal-400">
                          Otomatis ditambahkan ke galeri
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-xs text-charcoal-400 mt-1.5">
                      Pilih dari tema yang sudah pernah diinput
                      atau ketik tema baru. Hanya untuk produk
                      undangan digital atau cetak.
                    </p>
                  </div>
                </div>
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-charcoal-100 sticky top-0 bg-white z-10">
              <div>
                ...
                <p className="text-xs text-charcoal-400 mt-0.5">
                  {importedData.length} produk akan diproses
                </p>
              </div>
            ...
            <div className="p-6">
                    ...
                    {importedData.map(
                      (item, idx) => {
                        ...
                        return (
                          <tr
                            key={idx}
                            className="border-b border-charcoal-100/70"
                          >
                            ...
                            <td className="py-2 text-right text-charcoal-700">
                              {formatPrice(
                                item.price
                              )}
                            </td>
                            ...
                            <td className="py-2 text-center text-charcoal-600">
                              {item.stock === null
                                ? 'Tanpa batas'
                                : item.stock}
                            </td>
              ...
              <p className="text-xs text-charcoal-400 mb-4">
                Produk dengan nama yang sudah ada akan
                diperbarui. Produk baru akan ditambahkan.
              </p>
      ...
      {importResult && (
        ...
        >
          ...
          >
            ...
            <h3 className="font-serif text-lg font-bold text-charcoal-800 text-center">
              Import Selesai
            </h3>
      ...
      {confirmDelete && (
        ...
        >
          <div
            className="bg-white rounded-2xl max-w-sm w-full p-6 text-center animate-scale-in"
            onClick={(e) =>
              e.stopPropagation()
            }
          >
            ...
            <p className="text-sm text-charcoal-400 mt-2">
              "{confirmDelete.name}" akan dihapus permanen.
            </p>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="btn bg-navy-600 text-white hover:bg-navy-700 flex-1"
              >
                Hapus
              </button>
            </div>
